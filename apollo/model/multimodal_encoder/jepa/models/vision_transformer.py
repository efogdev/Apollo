...
from apollo.model.multimodal_encoder.jepa.models.utils.patch_embed import PatchEmbed, PatchEmbed3D
from apollo.model.multimodal_encoder.jepa.models.utils.modules import Block
from apollo.model.multimodal_encoder.jepa.models.utils.pos_embs import get_2d_sincos_pos_embed, get_3d_sincos_pos_embed
from apollo.model.multimodal_encoder.jepa.utils.tensors import trunc_normal_
...
class VisionTransformer(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if hasattr(self, 'pos_embed') and self.pos_embed is not None:
            self.device = self.pos_embed.device
        else:
            self.device = torch.device(fn(torch.tensor(0, device=self.device)).device)
    ...
    def save_pretrained(self, save_directory, state_dict):
        ...
        torch.save(state_dict, os.path.join(save_directory, "model.pth.tar"))
