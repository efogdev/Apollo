...
def transform_to_dict(transform):
    ...
    for t in transform.transforms:
        ...
        if hasattr(t, '__dict__'):
            params = {k: v for k, v in t.__dict__.items() if not callable(v)}
        transform_dict[name] = params
    return transform_dict


class ProcessorWrapper:
    def __init__(self, transform=None, processor=None, height=378, width=378,  frames_per_clip=1, image_mean = [0.48145466, 0.4578275, 0.40821073]):
