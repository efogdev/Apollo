...
class PretrainInternVideo2(PreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        ...
        for i in range(config.clip_return_layer):
            self.return_index.append(depth - int(i * config.clip_student_return_interval) - 1)
        ...
        self.patch_embed = PatchEmbed(
            config.img_size, config.patch_size, in_chans, embed_dim,
            num_frames=num_frames, tubelet_size=self.tubelet_size,
        )
        ...
        self.clip_projector = AttentionPoolingBlock(
            dim=embed_dim, num_heads=config.attn_pool_num_heads, qkv_bias=True, qk_scale=None,
...
def pretrain_internvideo2_1b_patch14_224(config):
    return PretrainInternVideo2(config)
