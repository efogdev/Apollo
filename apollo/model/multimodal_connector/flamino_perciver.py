...
class FlamingoResampler(nn.Module):
    def __init__(self, connector_config):
        super().__init__()

        num_latents = connector_config.num_output_tokens
        dim =  connector_config.text_hidden_size
        num_media_embeds = connector_config.num_key_value_heads
        dim_head = connector_config.resampler_head_dim
        heads = connector_config.resampler_n_heads
        ff_mult = 4

        self.latents = nn.Parameter(torch.randn(num_latents, dim))
        depth = connector_config.resampler_depth
