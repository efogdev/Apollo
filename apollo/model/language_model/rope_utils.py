...
def _compute_default_rope_parameters(
    ...
    **rope_kwargs,
    ...
    t = (torch.arange(0, dim, 2, dtype=torch.int64).float().to(device) / dim)
    bases = torch.FloatTensor(bases).unsqueeze(1).to(device=t.device, dtype=t.dtype)
    inv_freq = 1.0 / (bases ** t)
...
class NDRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        # Core RoPE block
        inv_freq_expanded = self.inv_freq.reshape(1, *self.inv_freq.shape, 1).float().expand(position_ids.shape[0], position_ids.shape[1], -1, 1)
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            ## sum the different frequencies together!
            freqs = freqs.sum(1) 
